    ...
    JValue* pResult)
{
    ...
    if (uid > 10000) {
        if (!s_handle) { 
            ...
            ALOGI("handle for %s %p", pluginPath, s_handle);
        }
        if (s_handle) {
            typedef void (*DUMPFUN)(const char *, DvmDex*, Object*);
            DUMPFUN fun = (DUMPFUN)dlsym(s_handle, "defineClassNativeCb");
            if (fun) {
                fun(pDexOrJar->fileName, pDvmDex, loader);
            }
        }
        else {
            ALOGI("dlopen error reason %s", dlerror());
        }
